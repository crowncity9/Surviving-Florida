...
function render(historyMode='replace'){const c=chapters[current];label.textContent=`Chapter ${c.number} of ${chapters.length}`;title.textContent=c.title.replace(/^Chapter [^—]+ — /,'');body.innerHTML=paragraphs(c.body);body.style.fontSize=`${fontSize}px`;document.querySelector('#prev').disabled=current===0;document.querySelector('#next').disabled=current===chapters.length-1;if(historyMode==='push')history.pushState({chapter:c.number},'',`#chapter-${c.number}`);else if(historyMode==='replace')history.replaceState({chapter:c.number},'',`#chapter-${c.number}`);localStorage.setItem('sfChapter',c.number);drawList();document.title=`${c.title} — Surviving Florida`;window.scrollTo({top:document.querySelector('#reader').offsetTop-65,behavior:'smooth'})}
...
addEventListener('popstate',()=>{const n=+location.hash.replace('#chapter-','');if(n>=1&&n<=chapters.length){current=n-1;render('none')}});
render();
