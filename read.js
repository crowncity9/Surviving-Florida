...
const AMAZON_URL='https://www.amazon.com/dp/B0HGKL1WRP';
const requestedChapter=+location.hash.replace('#chapter-','');
if(requestedChapter>FREE_CHAPTERS&&requestedChapter<=21)location.replace(AMAZON_URL);
...
function drawList(){list.innerHTML=chapters.slice(0,FREE_CHAPTERS).map((c,i)=>`<button class="chapter-link${i===current?' active':''}" data-i="${i}"><span>${String(c.number).padStart(2,'0')}</span><b>${esc(c.title.replace(/^Chapter [^—]+ — /,''))}</b></button>`).join('')+`<a class="chapter-buy" href="${AMAZON_URL}" target="_blank" rel="noopener">Get Chapters 4–21 on Amazon →</a>`;list.querySelectorAll('button').forEach(b=>b.onclick=()=>select(+b.dataset.i))}
function purchasePanel(){return `<div class="purchase-gate"><p class="chapter-label">Continue the story</p><h3>Chapters 4–21 are in the complete edition.</h3><p>Own the full <em>Surviving Florida</em> book and follow the war for Palm Haven through Chapter 21.</p><a class="buy-button" href="${AMAZON_URL}" target="_blank" rel="noopener">Get the complete book on Amazon</a></div>`}
function render(historyMode='replace'){if(current>=FREE_CHAPTERS)current=FREE_CHAPTERS-1;const c=chapters[current];label.textContent=`Free preview • Chapter ${c.number} of ${FREE_CHAPTERS}`;title.textContent=c.title.replace(/^Chapter [^—]+ — /,'');body.innerHTML=paragraphs(c.body)+(current===FREE_CHAPTERS-1?purchasePanel():'');body.style.fontSize=`${fontSize}px`;document.querySelector('#prev').disabled=current===0;document.querySelector('#next').disabled=current===FREE_CHAPTERS-1;if(historyMode==='push')history.pushState({chapter:c.number},'',`#chapter-${c.number}`);else if(historyMode==='replace')history.replaceState({chapter:c.number},'',`#chapter-${c.number}`);localStorage.setItem('sfChapter',c.number);drawList();document.title=`${c.title} — Surviving Florida`;window.scrollTo({top:document.querySelector('#reader').offsetTop-65,behavior:'smooth'})}
function select(i){current=i;const c=chapters[current];window.sfTrack?.('chapter_select',{chapter_number:c.number,chapter_title:c.title.replace(/^Chapter [^—]+ — /,'')});aside.classList.remove('open');document.querySelector('#chaptersBtn').setAttribute('aria-expanded','false');render('push')}
...
addEventListener('popstate',()=>{const n=+location.hash.replace('#chapter-','');if(n>=1&&n<=FREE_CHAPTERS){current=n-1;render('none')}});
render();
